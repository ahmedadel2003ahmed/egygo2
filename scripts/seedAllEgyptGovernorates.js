...
async function ensureIndexes() {
  try {
    ...
    try {
      ...
      console.log("   ✓ Province indexes ensured");
    } catch (err) {
      // Indexes may already exist, just warn
      console.log(
        "   ⚠ Province indexes already exist or conflict (continuing...)"
      );
    }
    ...
    try {
      ...
      console.log(
        "   ✓ Place indexes ensured (including 2dsphere for geospatial queries)"
      );
    } catch (err) {
      console.log(
        "   ⚠ Place indexes already exist or conflict (continuing...)"
      );
    }
  } catch (error) {
    console.error("   ❌ Unexpected error with indexes:", error.message);
